...
const getHitPower = (fighter) => {
  const hitPower = Number.parseFloat(fighter.attack * pounchChances(1, 2)).toFixed(2);
  return hitPower;
};

const getBlockPower = (fighter) => {
  const blockPower = Number.parseFloat(fighter.defense * pounchChances(1, 2)).toFixed(2);
  return blockPower;
};
